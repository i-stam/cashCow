...
contract RedirectAll is SuperAppBase, CFALibrary {
        ...
        returns (bytes memory newCtx)
    {
      ...
      if(inFlowRate > outFlowRate) {
          ...
          while(liens[ID].currentFlowRate != liens[ID].flowRate && ID <= lastId){
              ...
              (, lienFlow,,) = _cfa.getFlow(_acceptedToken, address(this), ERC721.ownerOf(ID));
      ...
      if(inFlowRate > totalLiens){
          ...
          while (liens[ID].flowRate >= (flowToReduce)) {
              if(liens[ID].flowRate > flowToReduce){
                ...
                //updateFlow;
                return newCtx = updateFlow(ERC721.ownerOf(ID), liens[ID].flowRate - flowToReduce, newCtx);
              } else{
                ...
                if(lienFlow == liens[ID].flowRate){
                    newCtx = _deleteFlow(address(this), ERC721.ownerOf(ID),newCtx);
                } else {
                    newCtx = _updateFlow(ERC721.ownerOf(ID), lienFlow - liens[ID].flowRate , newCtx)
                }
              }
              if(ID > 0) ID--;
    ...
    function _moveLien(address newReceiver, uint256 tokenId ) internal {
        ...
        address oldOwner = ERC721.ownerOf(ID);
